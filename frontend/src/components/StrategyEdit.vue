  ...
  return computed({
    get: () => {
      ...
      if (!Array.isArray(strategy)) {
        //console.log(`makeStrategyField(${type}, ${field}) get: no strategy array, returning default`, defaultValue)
        return defaultValue
      }
      const item = strategy.find(s => s?.type === type)
      const result = item?.parameters?.[field] ?? defaultValue
      //console.log(`makeStrategyField(${type}, ${field}) get:`, result, 'item:', item)
      return result
    },
    set: (value) => {
      //console.log(`makeStrategyField(${type}, ${field}) set:`, value)
...
// Update parameters from JSON (called when user edits the JSON textarea)
        ...
        <div v-if="strategyTab === 'general'" class="tab-pane">
          ...
          <div class="form-group">
            ...
            <textarea 
              id="stock-list"
              v-model="stockList"
              placeholder="Enter stock symbols separated by commas (e.g., AAPL, TSLA, MSFT) or use Strategy Parameters: Screening > stock universe settings"
              rows="4"
            ></textarea>
            <small class="form-help">
              List of stocks to apply this strategy to. Leave empty to use stock universe from Strategy Parameters.
            </small>
          </div>
        </div>
