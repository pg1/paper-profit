...
class ClaudeService:
    """Claude AI Platform service for financial analysis and insights"""
    ...
    def __init__(self, db_session: Session):
        ...
        logger.info(f"Claude API key loaded: {'***' + self.api_key[-4:] if self.api_key != 'demo' else 'demo key'}")
    
    def _get_api_key_from_settings(self):
        """Get Claude API key from settings table"""
        try:
            setting = self.repo.settings.get_by_name('Claude')
            if setting and setting.parameters:
                ...
                logger.info(f"Loaded Claude API key from settings table")
                return api_key
            else:
                logger.warning("Claude setting not found in database, using demo key")
                return 'demo'
        except Exception as e:
            logger.error(f"Error loading Claude API key from settings: {e}")
            return 'demo'
    ...
    def _make_api_request(self, endpoint: str, payload: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Request error calling Claude API: {e}")
            return None
        except Exception as e:
        ...
        if endpoint == "messages":
            ...
            mock_responses = {
                ...
                "content": [
                    {
                        "type": "text",
                        "text": f"Based on my analysis of {symbol}, I observe:\n\n"
    ...
    def analyze_stock(self, symbol: str, analysis_type: str = "comprehensive") -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            payload = {
                ...
                "system": "You are a financial analyst specializing in stock market analysis. Provide clear, data-driven insights with actionable recommendations."
            }
            ...
            if response and 'content' in response and len(response['content']) > 0:
                ...
                logger.info(f"Generated {analysis_type} analysis for {symbol} using Claude")
                return structured_analysis
            else:
                logger.warning(f"No valid response from Claude for {symbol} analysis")
                return None
                
        except Exception as e:
            logger.error(f"Error analyzing {symbol} with Claude: {e}")
            return None
    ...
    def generate_trading_strategy(self, symbol: str, timeframe: str = "short_term") -> Optional[Dict[str, Any]]:
        """Generate trading strategy using Claude AI"""
        try:
            ...
            payload = {
                ...
                "system": "You are an expert trading strategist. Provide specific, actionable trading strategies with clear risk management guidelines."
            }
            ...
            if response and 'content' in response and len(response['content']) > 0:
                ...
                logger.info(f"Generated {timeframe} trading strategy for {symbol} using Claude")
                return strategy_data
            else:
                logger.warning(f"No valid response from Claude for {symbol} strategy")
                return None
                
        except Exception as e:
            logger.error(f"Error generating trading strategy for {symbol} with Claude: {e}")
            return None
    
    def get_market_insights(self, sector: str = None) -> Optional[Dict[str, Any]]:
        """Get general market insights using Claude AI"""
        try:
            if sector:
            ...
            payload = {
                ...
                "system": "You are a market analyst providing broad market insights. Focus on actionable intelligence and emerging trends."
            }
            ...
            if response and 'content' in response and len(response['content']) > 0:
                ...
                logger.info(f"Generated market insights for {sector or 'general market'} using Claude")
                return insights_data
            else:
                logger.warning(f"No valid response from Claude for market insights")
                return None
                
        except Exception as e:
            logger.error(f"Error getting market insights with Claude: {e}")
            return None
    
    def compare_stocks(self, symbols: List[str], comparison_type: str = "performance") -> Optional[Dict[str, Any]]:
        """Compare multiple stocks using Claude AI"""
        try:
            ...
            payload = {
                ...
                "system": "You are a comparative stock analyst. Provide clear comparisons highlighting relative strengths and weaknesses."
            }
            ...
            if response and 'content' in response and len(response['content']) > 0:
                ...
                logger.info(f"Generated comparison for {symbol_list} using Claude")
                return comparison_data
            else:
                logger.warning(f"No valid response from Claude for stock comparison")
                return None
                
        except Exception as e:
            logger.error(f"Error comparing stocks {symbols} with Claude: {e}")
            return None
