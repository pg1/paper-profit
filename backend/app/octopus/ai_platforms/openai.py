...
class OpenAIService:
    """OpenAI Platform service for financial analysis and insights"""
    ...
    def __init__(self, db_session: Session):
        ...
        logger.info(f"OpenAI API key loaded: {'***' + self.api_key[-4:] if self.api_key != 'demo' else 'demo key'}")
    
    def _get_api_key_from_settings(self):
        """Get OpenAI API key from settings table"""
        try:
            setting = self.repo.settings.get_by_name('OpenAI')
            if setting and setting.parameters:
                ...
                logger.info(f"Loaded OpenAI API key from settings table")
                return api_key
            else:
                logger.warning("OpenAI setting not found in database, using demo key")
                return 'demo'
        except Exception as e:
            logger.error(f"Error loading OpenAI API key from settings: {e}")
            return 'demo'
    ...
    def analyze_stock(self, symbol: str, analysis_type: str = "comprehensive") -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if response and 'choices' in response and len(response['choices']) > 0:
                ...
                logger.info(f"Generated {analysis_type} analysis for {symbol} using OpenAI")
                return structured_analysis
            else:
                logger.warning(f"No valid response from OpenAI for {symbol} analysis")
                return None
                
        except Exception as e:
            logger.error(f"Error analyzing {symbol} with OpenAI: {e}")
            return None
    ...
    def generate_trading_strategy(self, symbol: str, timeframe: str = "short_term") -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if response and 'choices' in response and len(response['choices']) > 0:
                ...
                logger.info(f"Generated {timeframe} trading strategy for {symbol} using OpenAI")
                return strategy_data
            else:
                logger.warning(f"No valid response from OpenAI for {symbol} strategy")
                return None
                
        except Exception as e:
            logger.error(f"Error generating trading strategy for {symbol} with OpenAI: {e}")
            return None
    
    def get_market_insights(self, sector: str = None) -> Optional[Dict[str, Any]]:
        """Get general market insights using OpenAI"""
        try:
            if sector:
            ...
            if response and 'choices' in response and len(response['choices']) > 0:
                ...
                logger.info(f"Generated market insights for {sector or 'general market'} using OpenAI")
                return insights_data
            else:
                logger.warning(f"No valid response from OpenAI for market insights")
                return None
                
        except Exception as e:
            logger.error(f"Error getting market insights with OpenAI: {e}")
            return None
    
    def compare_stocks(self, symbols: List[str], comparison_type: str = "performance") -> Optional[Dict[str, Any]]:
        """Compare multiple stocks using OpenAI"""
        try:
            ...
            if response and 'choices' in response and len(response['choices']) > 0:
                ...
                logger.info(f"Generated comparison for {symbol_list} using OpenAI")
                return comparison_data
            else:
                logger.warning(f"No valid response from OpenAI for stock comparison")
                return None
                
        except Exception as e:
            logger.error(f"Error comparing stocks {symbols} with OpenAI: {e}")
            return None
