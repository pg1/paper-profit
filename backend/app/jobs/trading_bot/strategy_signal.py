...
class StrategySignal:
    ...
    def parse_strategy_parameters(self, strategy: Strategy) -> Dict[str, Any]:
        ...
        # Ensure entryConditions exists (from frontend Entry tab)
        if 'entryConditions' not in params:
            params['entryConditions'] = []
        
        # Ensure strategy array exists (from frontend Exit tab)
        if 'strategy' not in params:
            params['strategy'] = []
        
        # Ensure signalMapping exists (auto-generated by frontend)
        if 'signalMapping' not in params:
            params['signalMapping'] = {}
        
        # Set default values for required parameters (only if not already set)
        defaults = {
    ...
    # ── Condition-based Entry/Exit evaluation (merged from simplified_ralph_bot.py) ──
    
    def get_simple_market_data(self, symbol: str) -> Dict[str, Any]:
        """Get basic market data for a symbol (for condition-based evaluation)"""
        try:
            instrument = self.repo_factory.instruments.get_by_symbol(symbol)
            if not instrument:
                logger.warning(f"Instrument not found for symbol: {symbol}")
            ...
            market_data_list = self.repo_factory.market_data.get_latest_by_symbol(symbol, '1day', limit=1)
            if not market_data_list:
                return {}
            
            market_data = market_data_list[0]
            
            # Simple result with basic data
            result = {
                'symbol': symbol,
            ...
            # Add mock indicator values for testing
            # In production, these would come from technical indicators repository
            result['rsi'] = 50.0  # Mock RSI value
            result['sma_20'] = float(market_data.close) * 0.95  # Mock SMA
            result['ema_10'] = float(market_data.close) * 0.97  # Mock EMA
    ...
    def evaluate_single_condition(self, condition: Dict[str, Any], market_data: Dict[str, Any]) -> bool:
        #print(market_data)
        #print(condition)
        ...
        if period and indicator in ['RSI', 'EMA', 'SMA', 'BB_UPPER', 'BB_LOWER', 'STOCH', 'ATR']:
            # For indicators with period, construct key like 'rsi_14'
            indicator_key = f"{indicator.lower()}_{period}"
        
        # Try to get indicator value
        indicator_value = market_data.get(indicator_key)
        #print(indicator_key)
        
        # If not found with period, try without period
        if indicator_value is None:
            indicator_value = market_data.get(indicator.lower())
        
        # If still not found, use close price as fallback for price-based indicators
        if indicator_value is None and indicator in ['CLOSE', 'BB_UPPER', 'BB_LOWER']:
            indicator_value = market_data.get('close', 0)
        #print(indicator_value)
        
        # If indicator value not found, return False
        if indicator_value is None:
            logger.debug(f"Indicator {indicator} not found in market data for {market_data.get('symbol')}")
            return False
    ...
    def evaluate_entry_conditions(self, conditions: List[Dict[str, Any]], market_data: Dict[str, Any]) -> Tuple[bool, List[str]]:
        #print(conditions)
        ...
        for i, condition in enumerate(conditions):
            condition_met = self.evaluate_single_condition(condition, market_data)
            condition_logic = condition.get('logic', 'AND')
            ...
            if i == 0:
                ...
                # Apply logic between current result and new condition
            ...
            current_logic = condition_logic
        
        return result, reasons
    
    def evaluate_exit_conditions(self, strategy_params: Dict[str, Any], 
                                position_data: Dict[str, Any], 
                                market_data: Dict[str, Any]) -> Tuple[bool, str]:
        """Evaluate exit conditions from Exit tab"""
        ...
        for rule in strategy_params.get('strategy', []):
            if rule.get('type') == 'exit':
                exit_rule = rule.get('parameters', {})
                break
        
        if exit_rule:
            indicator = exit_rule.get('indicator', '')
            operator = exit_rule.get('operator', '')
            value = exit_rule.get('value', '')
            
            if indicator and operator and value:
                try:
                    # Get indicator value
                    indicator_value = market_data.get(indicator.lower(), 0)
                    if indicator_value == 0 and indicator == 'Close':
                        indicator_value = current_price
                    
                    value_float = float(value)
                    
                    # Check condition
                    if operator == '<' and indicator_value < value_float:
                        return True, f"Exit: {indicator} {operator} {value}"
                    elif operator == '>' and indicator_value > value_float:
                        return True, f"Exit: {indicator} {operator} {value}"
                    elif operator == 'crosses above' and indicator_value > value_float:
                        return True, f"Exit: {indicator} {operator} {value}"
                    elif operator == 'crosses below' and indicator_value < value_float:
                        return True, f"Exit: {indicator} {operator} {value}"
                except (ValueError, TypeError):
                    pass
        ...
        stop_loss_rule = None
        for rule in strategy_params.get('strategy', []):
            if rule.get('type') == 'stopLoss':
                stop_loss_rule = rule.get('parameters', {})
                break
        
        if stop_loss_rule and position_data:
            loss_type = stop_loss_rule.get('lossType', 'percent')
            value = stop_loss_rule.get('value', '')
            
            if value:
                try:
                    entry_price = position_data.get('average_entry_price', 0)
                    if entry_price > 0:
                        if loss_type == 'percent':
                            stop_loss_price = entry_price * (1 - float(value) / 100)
                            if current_price <= stop_loss_price:
                                return True, f"Stop Loss: {value}% below entry"
                        else:  # fixed amount
                            stop_loss_price = entry_price - float(value)
                            if current_price <= stop_loss_price:
                                return True, f"Stop Loss: ${value} below entry"
                except (ValueError, TypeError):
                    pass
        ...
        for rule in strategy_params.get('strategy', []):
            if rule.get('type') == 'takeProfit':
                take_profit_rule = rule.get('parameters', {})
                break
        
        if take_profit_rule and position_data:
            profit_type = take_profit_rule.get('profitType', 'percent')
            value = take_profit_rule.get('value', '')
                ...
                try:
                    ...
                    if entry_price > 0:
                        if profit_type == 'percent':
                            take_profit_price = entry_price * (1 + float(value) / 100)
                            if current_price >= take_profit_price:
                                return True, f"Take Profit: {value}% above entry"
                        else:  # fixed amount
                            take_profit_price = entry_price + float(value)
                            if current_price >= take_profit_price:
                                return True, f"Take Profit: ${value} above entry"
                except (ValueError, TypeError):
                    pass
    ...
    def generate_condition_based_signal(self, account: Account, strategy: Strategy, 
                                        instrument: Instrument, market_data: Dict[str, Any],
                                        strategy_params: Dict[str, Any],
                                        position_data: Optional[Dict[str, Any]] = None) -> Optional[Dict[str, Any]]:
        """Generate trading signal using condition-based Entry/Exit evaluation.
        
        This is an alternative to the scoring-based generate_trading_signal().
        It directly evaluates entryConditions and exit/stopLoss/takeProfit rules
        from the frontend strategy parameters.
        
        Returns a signal dict with 'action' ('BUY'/'SELL'/'HOLD'), 'price', 'reason'.
        """
        symbol = instrument.symbol
        current_price = market_data.get('close', 0)
        ...
        # If we have position data, check exit conditions first
        if position_data:
            should_exit, exit_reason = self.evaluate_exit_conditions(
                strategy_params, position_data, market_data
            )
            if should_exit:
                return {
                    'action': 'SELL',
        ...
        if not position_data:
            ...
            should_enter, entry_reasons = self.evaluate_entry_conditions(
                entry_conditions, market_data
            )
